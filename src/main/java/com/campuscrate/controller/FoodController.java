...
public class FoodController {
    ...
    @PostMapping("/api/food/payments/sslcommerz/success") public ResponseEntity<Void> paymentSuccess(@RequestParam Map<String,String> values){return redirect(food.completeOnlinePayment(values.get("tran_id"),values.get("val_id"),true) ? "payment=success" : "payment=failed");}
    @PostMapping("/api/food/payments/sslcommerz/fail") public ResponseEntity<Void> paymentFail(@RequestParam Map<String,String> values){food.completeOnlinePayment(values.get("tran_id"),null,false);return redirect("payment=failed");}
    @PostMapping("/api/food/payments/sslcommerz/cancel") public ResponseEntity<Void> paymentCancel(@RequestParam Map<String,String> values){food.completeOnlinePayment(values.get("tran_id"),null,false);return redirect("payment=cancelled");}
